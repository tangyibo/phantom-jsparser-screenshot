...
if (system.args.length !== 2) {
  console.log('Usage: webpage_fetcher.js <portnumber>');
  phantom.exit(1);
} else {
  port = system.args[1];
  ...
  service = server.listen(port, {
    ...
    page.onLoadFinished = function(status) {
      ...
      setTimeout(make_result, wait_before_end+20, page);
    };
    ...
    function _make_result(page) {
      ...
      var filename=null;
      var srcreenshot=fetch.make_screen ? true : false;
      if(first_response.status === 200 && srcreenshot ) {
          filename='snapshots/' + urlToFile(fetch.url)+'.jpg';
          page.render(filename , {format: 'jpeg', quality: '50'}); 
      }
	...
	.replace(/[\.|/|?|&]/g, '_')
      ...
      return prefix + "_" + date.join('-') + '_' + time.join('-');
    }
  });
